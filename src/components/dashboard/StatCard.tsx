    ...
    <Card className="bg-white shadow-sm hover:shadow-md transition-shadow">
      ...
      <CardContent>
        <div className="flex items-center gap-4">
          <div>
            <div className="text-4xl font-bold text-gray-900">{current}</div>
            <div className="text-sm text-gray-500">out of {max}</div>
          </div>
          <div className="ml-auto text-3xl font-semibold text-gray-400">
            {percentage}%
          </div>
        </div>
        {title !== "Team Members" && (
          <Progress value={percentage} className="h-1 mt-4" />
        )}
      </CardContent>
    </Card>
  )
